...
(function () {
    ...
    const elementSortValue = (el) => serverChoicesSortOrder[el.innerText.trim()];
    const select = document.getElementById('selectServer');
    if (select && serverChoices.includes(select.selectedOptions[0].innerText.trim()))
        return;
    ...
    if (window.location.href.slice(window.location.origin.length) === feServerUrl)
        return;
